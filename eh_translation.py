# -*- coding: utf-8 -*-
"""EH-translation.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/12yFh0h3H5n7QkZzNU6ZXDEMVN0NEKg8F
"""
...
"""**randgenerate_sequences_together** takes random window of 1/4th the length to be used as training data"""

import pandas as pd
import numpy as np

def randgenerate_sequences_together(user_type,win_len, samples):
  resulteye=[]
  resulthand=[]
  maxlen=0
  eye_basic_url='/content/drive/My Drive/data_set/Eye_'
    ...
    for j in range(0,2):
      ...
      try:
        ...
        for i in range(samples):
          random_int = np.random.randint(1, len(dataeye)-int(len(dataeye)//4))
          resulteye.append(dataeye[random_int:random_int+int(len(dataeye)//4)])
          resulthand.append(datahand[random_int:random_int+int(len(dataeye)//4)])
#        resulteye.append(dataeye)
#        resulthand.append(datahand)
        if len(dataeye)>maxlen or len(datahand)>maxlen:
          maxlen=max(int(len(dataeye)//4), int(len(datahand)//4))
      except IOError:
        print("")
  return resulteye, resulthand, maxlen

Ai, Ao, Am =randgenerate_sequences_together('TD',1000,10)

Bi, Bo, Bm =randgenerate_sequences_together('ASD',1000,10)

modeltd, encoder_modeltd, decoder_modeltd, scalertd, final_losstd = LSTM_model(Bi, Bo, Bm, 2,60)

"""**filtereddata** only takes data in a certain window"""

def filtereddata(urleye, urlhand, Xmain, Ymain, windowlength):
  unfilteredeye, unfilteredhand = handeyetogether(urlhand, urleye, windowlength)
  badrows=[]
  for i in range(len(unfilteredeye)):
    if unfilteredeye[i][0]>Xmain or unfilteredeye[i][1]>Ymain:
      badrows.append(i)
  filteredeye = np.delete(unfilteredeye, badrows, axis=0)
  filteredhand = np.delete(unfilteredhand, badrows, axis=0)
  return filteredeye, filteredhand
filtereddata('/content/drive/My Drive/data_set/Eye_TD_U1_Active_1.xlsx', '/content/drive/My Drive/data_set/Hand_TD_U1_Active_1.xlsx', 0.3, 0.3, 1000)
  ...
  for i in range(1, numOfUser+1):
    for j in range(0,2):
      ...
      try:
        ...
        print("")
  return resulteye, resulthand, maxlen
overlapeye, overlaphand, overlapmax = geneyehandoverlaps('ASD')
...
def randeyehandoverlaps(user_type, samples):
  resulteye=[]
  resulthand=[]
  maxlen=0
  eye_basic_url='/content/drive/My Drive/data_set/Eye_'
  ...
  for i in range(1, numOfUser+1):
    for j in range(0,2):
      ...
      #asd_eye_data=pd.DataFrame()
      try:
        dataeye, datahand =eyehandoverlaps(c_hand_url, c_eye_url,)
        for i in range(samples):
          random_int = np.random.randint(1, len(dataeye)-int(len(dataeye)//2))
          resulteye.append(dataeye[random_int:random_int+int(len(dataeye)//2)])
          resulthand.append(datahand[random_int:random_int+int(len(dataeye)//2)])
#        resulteye.append(dataeye)
#        resulthand.append(datahand)
        if len(dataeye)>maxlen or len(datahand)>maxlen:
          maxlen=max(int(len(dataeye)//2), int(len(datahand)//2))
      except IOError:
        print("")
  return resulteye, resulthand, maxlen

"""Comparing how the window length predictions are for ASD and TD. TD has a higher error of about 2x."""

input, output, max_len=generate_sequences_together('ASD',1000)
LSTM_model(input, output, max_len, 2, 30)

"""Considering different windows of testing"""

input, output, max_len=generate_sequences_together('ASD',2000)
LSTM_model(input, output, max_len, 2, 30)

input, output, max_len=generate_sequences_together('TD',1000)
LSTM_model(input, output, max_len, 2, 30)

"""Running the LSTM on unfiltered data"""
